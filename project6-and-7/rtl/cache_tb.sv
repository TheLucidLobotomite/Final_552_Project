module cache_tb ();
    ...
    task automatic send_read(input reg [22:0] tag, input reg [4:0] index, input reg [3:0] offset, input reg [3:0] mask);
        begin
            ...
            if (!busy) begin
                $display("Read HIT:   addr=%08x data=%08x mask=%04b at time %0t", req_addr, res_rdata & expanded_mask, req_mask, $time);
            end else begin
                while (busy) @(negedge clk);
                $display("Read MISS:  addr=%08x data=%08x mask=%04b at time %0t", req_addr, res_rdata & expanded_mask, req_mask, $time);
            end
        end
    endtask
